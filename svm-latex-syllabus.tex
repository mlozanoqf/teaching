...
E-mail: $if(email)$\texttt{$email$}$else$TBD$endif$ & Personal: $if(Personal)$\href{http://$Personal$}{\tt $Personal$}$else$TBD$endif$\\
Universidad de Monterrey (UDEM) $if(Universidad)$$Universidad$$else$ $endif$  &  Web: $if(Web)$\href{http://$Web$}{\tt $Web$}$else$TBD$endif$\\
Department: $if(Department)$$Department$$else$Finance, Economics$endif$  & Semester: $if(Semester)$$Semester$$else$TBD$endif$\\
	&  \\
	\hline
	\end{tabular*}
